...
export async function POST(request: Request) {
  const backendUrl = resolveBackendVerifyInitDataUrl()
...
function resolveBackendVerifyInitDataUrl(): string | null {
  const baseUrl = (process.env.BACKEND_API_BASE_URL ?? BACKEND_DEFAULT_API_BASE_URL).trim()
  const normalizedBaseUrl = baseUrl.replace(/\/+$/, '')

  try {
    return new URL(`${normalizedBaseUrl}/auth/telegram/verify-init-data`).toString()
  } catch {
    return null
  }
}
